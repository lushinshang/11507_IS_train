...
- **資訊圖表 prompt 佔位**：適合大量資訊一次呈現的段落（例如多重面向的比較清單），先放一段標示「🖼️ 資訊圖表建議（AI 產圖 prompt，尚未產圖，先放文字佔位）」的 blockquote，內容是給圖片生成工具的英文 prompt 說明，不實際產圖。
...
- 適合大量資訊一次呈現的段落（多重面向比較清單、循環週期等），先放一段「🖼️ 資訊圖表建議（AI 產圖 prompt，尚未產圖，先放文字佔位）」的 blockquote，內容是給圖片生成工具的英文 prompt，不要真的產圖。
