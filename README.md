...
目前已完成「第二期 Day1」「第二期 Day2」各四堂課程導讀與整合首頁。
...
├── Day2/               第二期 Day2 課程產出（已完成，結構同 Day1，製作方式見「Day2 製作 SOP」）
└── _tooling/           非課程內容的工具產出，與課程資料分開放，方便瀏覽專案時聚焦內容
    ├── playwright-mcp/     Playwright 測試工具的 log 與頁面快照（開發過程留存，非交付物）
    └── qa/                 根目錄與各 Day 的 RWD 驗證截圖（qa_root_index_*.png、qa_day2_index_*.png 等）
```
...
- **Day2 卡片**：可點擊，連到 `Day2/index.html`。
...
`Day2/index.html` 已比照 Day1 內嵌 Day2 課程影片播放器，來源為 GitHub Release 重新提供的版本：<https://github.com/lushinshang/11507_IS_train/releases/download/mv/115.Day2.mp4>（2026-07-18 已實測連結回應 200）。

- 播放器結構、`preload="none"`、來源標示與相容性說明皆與 Day1 相同（見上方「Day1 課程影片」）。
- Day2 首頁原本的「📹 課程影片製作中」提示已移除。

## Day2 來源

- 課程名稱：115 年中小企業基礎資安培訓課程｜第二期 Day2
- 講師：鄭郁翰副主任（崑山科技大學電腦與遊戲發展科學學士學位學程）；下午企業講座另邀魏國銳總監（三甲科技，逢甲大學資訊工程學系兼課教師）
- 官方課程主題：「客戶資料外洩了！守護企業機敏資料的風險管理竅門」
- 串流日期：2026 年 7 月 18 日
- 課程影片：<https://www.youtube.com/watch?v=Kc14Un4SFS8>（已比照 Day1 內嵌播放器，來源為 GitHub Release 重新提供的版本：<https://github.com/lushinshang/11507_IS_train/releases/download/mv/115.Day2.mp4>）
- 完整逐字稿（上午 09:00-12:00）：<https://hackmd.io/@lanss/S19ZY6_4Mx>
- 完整逐字稿（下午 13:00-16:00）：<https://hackmd.io/@lanss/BJdeF0ONGl>

逐字稿分上午、下午兩份取得（各約 370／347 行）。官方課程表把全天定位為同一大主題「核心資料機密保護與風險防範機制」分三段（上／下／情境模擬實作），中間插播一場標竿學習企業講座；四篇文章拆分即依此結構，並用逐字稿中實際的講者/主題轉折點核對邊界（例如「多因子驗證」明確從上午逐字稿第265行才展開，「情境模擬」明確從下午逐字稿第295行才開始分組討論），不是機械照官方時段表切。
...
| 編號 | 課程 | 講者 | 主要內容 |
| --- | --- | --- | --- |
| 01 | 核心資料機密保護與風險防範機制（上） | 鄭郁翰副主任 | 資料外洩定義、經營風險與法律責任、資訊資產盤點三問（內容/位置/誰能用）、分級授權、帳號權限控管常見盲點 |
| 02 | 核心資料機密保護與風險防範機制（下） | 鄭郁翰副主任 | 帳號安全與密碼弱點、多因子驗證（MFA）三型別、密碼管理器取捨、資安文化如何融入日常工作流程 |
| 03 | 標竿學習企業講座 | 魏國銳總監 | 以攻擊者視角盤點攻擊面、偵查到橫向移動的攻擊鏈、後台驗證機制自檢、社交工程演練實例、管理/技術/政策三軸自檢 |
| 04 | 核心資料機密保護與風險防範機制（實作） | 鄭郁翰副主任 | 資料外洩情境模擬討論、24小時/7天/30天/90天分級應變 SOP、剩餘（殘餘）風險概念 |

### Day2 完成與發布驗證紀錄

2026-07-18 已完成 Day2 發布版整理：根目錄總入口與 Day2 首頁均已開通，四篇導讀可直接閱讀；7 組資訊圖共 14 張 PNG 均已嵌入文章，桌機使用 16:9 版本、手機使用 9:16 版本，並支援點擊放大。最終檢查結果為：4 篇 HTML、7 個資訊圖 `<figure>`、7 個手機 `<source>`、14 張資訊圖、8 張桌機／手機全頁 QA 截圖，且發布版 HTML 中沒有殘留產圖 prompt 或「尚未產圖」佔位文字。四篇 HTML 與 Day2 首頁均已通過實際讀取內容的 Python `HTMLParser.feed()` 語法檢查。
...
### Day2 資訊圖表 prompt 與產圖記錄（已完成）

2026-07-18 已依下列 prompt 透過 `codex exec` 與內建 `imagegen` skill 完成 7 組資訊圖，共 14 張 PNG。每組包含 16:9 桌機版與 9:16 手機版，均已人工檢查主題、必要文字、視覺元素與尺寸；文章中的 prompt 佔位已替換為 `<figure class="infographic"><picture>…</picture></figure>`，並加入點擊放大 lightbox。文章01、03 原本各有一張 Mermaid 流程圖，因 pandoc 會把特定 raw HTML SVG 區塊錯誤包進 `<p>` 標籤而造成樣式跑掉，因此改用具 Mermaid 流程圖視覺感的資訊圖。

實際輸出檔案：

- 01：`01_data-inventory-flow.png`／`01_data-inventory-flow-mobile.png`、`01_defense-layers.png`／`01_defense-layers-mobile.png`
- 02：`02_mfa-comparison.png`／`02_mfa-comparison-mobile.png`
- 03：`03_attack-chain.png`／`03_attack-chain-mobile.png`、`03_backend-auth-check.png`／`03_backend-auth-check-mobile.png`、`03_three-axis-checklist.png`／`03_three-axis-checklist-mobile.png`
- 04：`04_response-timeline.png`／`04_response-timeline-mobile.png`

**01｜資料盤點三問流程圖**
> 扁平化流程示意圖，具備 Mermaid 流程圖的視覺感（圓角矩形節點＋箭頭連接線）：左側一個橘黃色圓角矩形節點標示中文「起點：你保護不了不知道的東西」，用三條箭頭分別指向中間三個藍色圓角矩形節點「問一：資料內容是什麼」「問二：資料放在哪裡」「問三：誰能存取使用」，三者再各自用箭頭匯流到右側一個綠色圓角矩形節點「依風險分級並設定對應權限」。簡潔商務資安風格，白色背景，節點文字清晰易讀，橫式 16:9 比例；手機直式版（9:16）把同樣五個節點與文字改為由上到下垂直堆疊排列，維持相同顏色配置與箭頭方向邏輯。

**01｜多層防禦同心環**
> 畫面呈現「多層防禦」概念：中央是一份文件圖示，外圍環繞四道同心防護環，分別標示中文標籤「電子郵件加密：附件外洩也看不到內容」「連結權限管控：連結外洩也無法開啟」「裝置遺失防護：電腦或手機遺失也讀不到資料」「密碼外洩防護：密碼外流也登不進帳號」。風格為簡潔的資安示意圖，藍灰配色，強調「單一防線失守不等於全面失守」的層層阻擋概念。

**02｜多因子驗證型式比較**
> 一張比較四種常見多因子驗證方式的資訊圖表，橫向排列「簡訊驗證碼」「驗證碼 App」「實體安全金鑰（USB／NFC）」「生物辨識（指紋／臉部）」四個欄位，每欄用簡潔圖示搭配三行中文標籤：「原理」「常見弱點」「適用情境」。例如簡訊驗證碼欄標示弱點為「可能遭 SIM 卡竊換或語音信箱漏洞攔截」；實體安全金鑰欄標示弱點為「離線較安全，但遺失會影響自己登入，需備援」。整體風格簡潔扁平、資安主題配色（藍／琥珀／綠），標題「多因子驗證常見型式比較」。

**03｜攻擊者視角攻擊鏈流程圖**
> 扁平化流程示意圖，具備 Mermaid 流程圖的視覺感（圓角矩形節點＋箭頭連接線），橫向排列五個依序連接的節點：藍色「偵查：從攻擊者視角盤點對外資產」→藍色「找入口：登入口、後台、對外服務」→橘黃色「取得與執行：社交工程或漏洞利用」→橘黃色「擴散：同仁誤轉發、內外網未切割」→紅色「橫向移動：控制關鍵系統與資料」。簡潔商務資安風格，白色背景，節點文字清晰易讀，橫式 16:9 比例；手機直式版（9:16）把同樣五個節點改為由上到下垂直堆疊排列，維持相同顏色配置與箭頭方向邏輯。

**03｜後台驗證機制四項自檢**
> 畫面內容：一張三欄式對照卡片，標題為「後台驗證機制四項自檢」。每欄用圖示＋簡短文字呈現一個檢查項目：①「密碼強度」圖示為盾牌加鎖頭，文字標籤「避免鍵盤排列型密碼」；②「多因子驗證 MFA」圖示為手機加驗證碼，文字標籤「單一密碼不足以防護」；③「登入錯誤鎖定與閒置帳號停權」圖示為沙漏加叉號，文字標籤「久未使用即停權」；④「閒置逾時登出」圖示為時鐘加登出符號，文字標籤「離開太久自動登出」。風格：扁平化資安風格插畫，藍灰配色，中文標籤清楚易讀，適合投影片或社群圖卡使用。

**03｜管理／技術／政策三軸自檢清單**
> 畫面內容：一張三軸雷達圖或三欄式清單卡片，標題為「管理面、技術面、政策面自檢清單」。管理面欄位標籤：帳號生命週期管理、權責分工、委外廠商資安條款；技術面欄位標籤：MFA、更新與修補、備份加密保護；政策面欄位標籤：書面規則公告、教育訓練落地、事件應變流程明文化。風格：三欄卡片式資訊圖，商務簡約風格，藍、橘、綠三色區分三個面向，中文標籤清楚。

**04｜24小時／7天／30天／90天應變對照表**
> 畫一張橫向對照表資訊圖，四欄分別對應「24 小時／7 天／30 天／90 天」，每欄下方列出該階段的行動性質關鍵字（例如 24 小時欄標示「鎖帳號、保留證據、隔離系統」；7 天欄標示「啟用 MFA、盤點影響範圍、補通報流程」；30 天欄標示「重新盤點權限、教育訓練、跨部門協調」；90 天欄標示「採購 DLP／端點防護、預算簽核、系統導入」）。風格建議用扁平化商務簡報風，四欄用由深紅到淺綠的漸層色區分緊急程度，中文標籤清晰易讀，整體呈現「時間軸＋行動強度遞減、投入資源遞增」的視覺對比。
...
主要成果位於 `Day1/` 與 `Day2/`。兩日結構與檔案用途相互對應；Day2 的 4 篇文章已完成 7 組、14 張桌機／手機資訊圖，存放在 `Day2/images/`。
...
└── Day1/
    ...
    ├── build_html.sh
    ├── images/              資訊圖表（AI 產圖，桌面版 + 9:16 手機版）
    └── qa_*.png
```
...
- `deepguide_style.html`：四篇文章共用的響應式閱讀樣式，含 Mermaid 圖表容器與資訊圖表 `<figure>` 樣式。
- `article_nav.html`：文章上方的 Day1 共用導覽。
- `article_footer.html`：文章頁尾與返回目錄入口。
- `build_html.sh`：Markdown 正規化、HTML 轉換與語法檢查流程。
- `images/`：四篇文章的 AI 產生資訊圖表，每個主題各兩張——桌面版（16:9 或 1:1）與手機直式版（`-mobile` 後綴，9:16），文章內用 `<picture>` 依螢幕寬度切換。完整產圖 prompt 記錄在上方「資訊圖表 prompt 記錄」。
...
4. 使用 Pandoc 產生獨立 HTML，並加上 `--metadata document-css=false`（見下方說明）。
...
**`document-css=false` 是必要的修復，不要移除**：Pandoc `--standalone` 模式預設會內嵌自己的一段 CSS，其中 `body{max-width:36em}` 會和 `deepguide_style.html` 疊加，把桌面版內文壓成約 612px 的窄欄、兩側留下大片空白（2026-07-18 發現並修復，此 bug 從最早期建置就存在）。加上 `--metadata document-css=false` 停用 Pandoc 內建樣式，只留自訂樣式表，桌面版才會正常滿版顯示。
...
- 文章內的資訊圖表用 `<picture>` 依螢幕寬度自動切換桌面版（16:9／1:1）與手機直式版（9:16），確保圖內文字在手機也清晰可讀，不需要 JS。
...
**注意**：`python3 -m html.parser <file>` 這個指令**沒有作用**——`html.parser` 模組沒有 `__main__` 進入點，這行指令不會讀取或解析檔案，對任何輸入（包括不存在的檔案）都恆回傳 exit code 0。這個問題從 Day1 最早期就存在，直到 2026-07-18 做 Day2 最終驗收時才被 fresh agent 發現。已修正 `build_html.sh`（Day1、Day2 兩份）與下方指令，改用會真的 `.feed()` 解析內容的寫法：

```bash
CHECK='python3 -c "from html.parser import HTMLParser; import sys; HTMLParser().feed(open(sys.argv[1], encoding=\"utf-8\").read()); print(\"OK:\", sys.argv[1])"'

eval $CHECK index.html
eval $CHECK Day1/index.html
eval $CHECK Day1/01_資安事件應變與防禦縱深_深度導讀.html
eval $CHECK Day1/02_中小企業資安學習與輔導資源_深度導讀.html
eval $CHECK Day1/03_不用熱血抗資安_AI與資安治理實務_深度導讀.html
eval $CHECK Day1/04_惡意程式攻防演示與網路分段_深度導讀.html
eval $CHECK Day2/index.html
eval $CHECK Day2/01_核心資料機密保護與風險防範機制（上）_深度導讀.html
eval $CHECK Day2/02_核心資料機密保護與風險防範機制（下）_深度導讀.html
eval $CHECK Day2/03_標竿學習企業講座_深度導讀.html
eval $CHECK Day2/04_核心資料機密保護與風險防範機制（實作）_深度導讀.html
```

這個寫法對不存在的檔案或讀檔失敗會丟出例外、exit code 非 0（已實測驗證），是真正有效的檢查，而非只是「跑起來沒報錯」。
...
根目錄總入口 `index.html` 的 RWD 驗證截圖存放於 `_tooling/qa/`（`qa_root_index_desktop.png`、`qa_root_index_mobile.png`），詳見「專案根目錄結構」一節。

Day2 的首頁 RWD 驗證存放於 `_tooling/qa/`（`qa_day2_index_desktop.png`、`qa_day2_index_mobile.png`）。2026-07-18 完成資訊圖後，四篇文章各新增桌機與手機全頁截圖並保存在 `Day2/`：`qa_01_desktop_images.png`／`qa_01_mobile_images.png` 至 `qa_04_desktop_images.png`／`qa_04_mobile_images.png`，用來確認 16:9／9:16 圖片切換、文字可讀性與版面無溢出。2026-07-19 加入 Day2 課程影片播放器後，新增 `_tooling/qa/qa_day2_index_video_desktop.png`、`qa_day2_index_video_mobile.png`，確認影片區塊在桌機（1440px）與手機（390px）皆無橫向溢出（`scrollWidth === clientWidth`）。
...
- **實際產圖**：用 `codex exec -C 專案路徑 --ephemeral "使用 imagegen skill，依照以下 prompt 產生 PNG，完成後複製到指定的 DayN/images/檔名.png……"` 產生圖片；每個不同主題或版型各呼叫一次 built-in `image_gen`。產圖前確認 Codex CLI 已登入且版本支援 `exec`。
...
- 產圖成功後，把 md 裡的 prompt blockquote 換成：
  ...
  ```
- 圖片存於對應的 `DayN/images/`；`<figure class="infographic">`／`<picture>`、桌機／手機比例與點擊放大 lightbox 的 CSS／JS 由 `deepguide_style.html` 和 `article_footer.html` 共用。
...
- 用 `Day1/build_html.sh` 的模式（複製一份改路徑，或參數化成可指定日期目錄）跑 Pandoc 產生 HTML，每次產出後對所有 HTML 執行「HTML 語法」一節記載的真實驗證指令（**不是** `python3 -m html.parser <file>`，那個指令沒有作用，見上方說明），全數 exit code 0 才算過。
