...
for source_md in "$script_dir"/0[1-4]_*.md; do
  ...
  python3 -c "from html.parser import HTMLParser; import sys; HTMLParser().feed(open(sys.argv[1], encoding='utf-8').read()); print('OK:', sys.argv[1])" "$output_base.html"
done

python3 -c "from html.parser import HTMLParser; import sys; HTMLParser().feed(open(sys.argv[1], encoding='utf-8').read()); print('OK:', sys.argv[1])" "$script_dir/index.html"
